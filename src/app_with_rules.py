...
if page == "Prédiction Individuelle":
        ...
        def safe_process_rules(column):
            try:
                return column.astype(str).apply(lambda x: 
                    ', '.join(ast.literal_eval(x)) if x != 'nan' else 'N/A'
                )
            except:
    ...
    if not filtered_rules.empty:
        ...
        display_rules['antecedents'] = display_rules['antecedents'].astype(str).apply(
            lambda x: ', '.join(ast.literal_eval(x)) if x != 'nan' else 'N/A'
        )
        display_rules['consequents'] = display_rules['consequents'].astype(str).apply(
            lambda x: ', '.join(ast.literal_eval(x)) if x != 'nan' else 'N/A'
        )
...
st.markdown(
    ...
    <div style='text-align: center; color: #666; padding: 1rem;'>
        <p>🎓 Dashboard de Prédiction du Risque d'Abandon Scolaire</p>
        <p>Développé avec Streamlit et Machine Learning</p>
    </div>
    """,
    unsafe_allow_html=True
)